...
private:
    ...
    int estado;        // SUCESSO=1, FALHA=0 (atende requisito 13/16 do PDF)

    void setUp();      // Configura ambiente de teste
    void tearDown();   // Limpa ambiente
    void testarCenarioValido();  // Teste com valores válidos (requisito 12)
    void testarCenarioInvalido();// Teste com valores inválidos (requisito 12)
    void testarAssociacaoCarteiras(); // Verifica vinculação Conta-Carteira (PDF: "listar carteiras associadas")

public:
    int run(); // Orquestra todos os testes (requisito 11)
};
#endif